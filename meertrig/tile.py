...
def hexagonGrid(beamNumber, beamRadius, subBeamRadius=None):
    ...
    for i in range((horizontalNumber - 1) / 2):
        coordinates += [
            [x, y + (i + 1) * 2 * verticalOffset] for x, y in twoLines[lineType]
        ]
        coordinates += [
            [x, y - (i + 1) * 2 * verticalOffset] for x, y in twoLines[lineType]
        ]
...
def ellipseCompact(beamNumber, axisH, axisV, angle, error, seed=None, write=False):
    ...
    beamRadius = np.sqrt(area / np.pi) * 1.0
    error = int(round(error / 2.0))
    if seed is None:
        random.seed(axisH)
    else:
        random.seed(seed)
    ...
    while abs(inCircleCount - (beamNumber + error)) > error:
        ...
        if trialCount > 150:
            if abs(inCircleCount - beamNumber) < int(beamNumber * 0.1):
                logger.warning(
                    "maximum trials reached in the tiling process, "
                ...
                # })
    logger.info(
        "tiling result, required_beam_number: {}, generate_beam_number: {}, "
...
def ellipseGrid(beamRadius, axisH, axisV, angle, write=False):
    ...
    if angle == 0:
        ...
        inCircleCoordinatesRotated = np.dot(
            np.array(rotationMatrix), np.array(inCircleCoordinates).T
        )
...
def squareGrid(beamNumber, beamRadius, subBeamRadius=None):
    sideLength = 2 * beamRadius
    if subBeamRadius is None:
        subBeamRadius = sqrt(sideLength * sideLength / beamNumber) / 2.0
...
def optimizeGrid(beamNumber, beamRadius, beamPattern, error, boreSight=[0, 0]):
    ...
    while abs(inCircleCount - beamNumber) > error:
        ...
        inCircleCoordinates, subBeamRadius = beamPattern(
            beamNumber, beamRadius, subBeamRadius * factor
        )
        inCircleCount = len(inCircleCoordinates)
        trialCount += 1
        if trialCount > 150:
    ...
    if (boreSight != [0, 0]).all():
        inCircleCoordinates = [
            [x + boreSight[0], y + boreSight[1]] for x, y in inCircleCoordinates
        ]
...
def main():
    ...
    coordinates, subBeamRadius = optimizeGrid(
        beamNumber, beamRadius, hexagonGrid, 5, beamBoreSight
    )
